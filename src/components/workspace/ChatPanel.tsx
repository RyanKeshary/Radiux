...
export function ChatPanel({
  ...
  return (
    ...
    >
      {/* Top Header Bar with Docking Action */}
      <div 
        className="px-3 py-2 border-b flex items-center justify-between text-xs flex-shrink-0"
        style={{ backgroundColor: 'var(--ide-dock-header)', borderColor: 'var(--ide-border)' }}
      >
        <div className="flex items-center gap-1.5 font-semibold text-slate-200">
          <MessageSquare className="w-3.5 h-3.5 text-sky-400" />
          <span>Team Chat</span>
          <span className="text-[10px] text-slate-400 font-normal">({messages.length})</span>
        </div>
        ...
        <div className="flex items-center gap-1">
          {isSidebarMode && onDockToBottom && (
            <button
              ...
              title="Open Chat in Bottom Terminal Dock"
            >
          ...
          {!isSidebarMode && onDockToSidebar && (
            <button
              ...
              className="p-1 px-2 rounded bg-sky-500/10 text-sky-400 hover:bg-sky-500/20 transition-colors flex items-center gap-1 text-[11px] font-medium"
              title="Move Chat to Left Sidebar"
            >
